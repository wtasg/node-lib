 ...
 */
function replaceAll(s, old, newStr) {
 ...
 */
function toValidUTF8(s, replacement) {
    let result = "";
    for (let i = 0; i < s.length; i++) {
...
function trimRight(s, cutset) {
    ...
    while (i >= 0) {
        const ch = runes[i];
        if (ch === undefined || !cutset.includes(ch)) {
            break;
        }
        end -= ch.length;
        i--;
...
class Reader {
    _encoded;
    _i = 0; // current byte position
    _prevRuneSize = -1; // byte size of last rune read (-1 if none)
    constructor(s) {
    ...
    seek(offset, whence) {
        ...
        switch (whence) {
            ...
            case 1:
                ...
                break;
            case 2:
                abs = this._encoded.length + offset;
                break;
            default: throw new Error("strings.Reader.seek: invalid whence");
        }
        if (abs < 0) {
            throw new RangeError("strings.Reader.seek: negative position");
        }
        this._i = abs;
        return abs;
    }
    ...
    unreadRune() {
        if (this._i <= 0) {
            throw new Error("strings.Reader.unreadRune: at beginning of string");
        }
        if (this._prevRuneSize < 0) {
            throw new Error("strings.Reader.unreadRune: previous operation was not ReadRune");
        }
