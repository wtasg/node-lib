 ...
 * ```
 */
function getAscii(input: string): string {
    return input
        .split("")
 ...
 * ```
 */
function hashString(input: unknown, salt?: string, size?: number): number {
    const hashInput = getAscii(String(input)).padEnd(
        size ?? LOWER_ALPHA.length,
        salt ?? LOWER_ALPHA,
    );
    return hashInput
        .split("")
